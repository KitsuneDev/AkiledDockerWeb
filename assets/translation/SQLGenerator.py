...
with open("www/swfs/nitro/gamedata/FurnitureData.json", encoding='utf-8') as f:
    data = json.load(f)
...
with open("www/swfs/nitro/gamedata/ProductData.json", encoding='utf-8') as f:
    data = json.load(f)
...
with open("catalog_items.sql", "w", encoding='utf-8') as f:
    for furni in known_names:
        ...
        if furni["name"] is None:
            continue
        furni_name = sqlescape(str(furni["name"]).encode("latin-1", "ignore").decode("latin-1"))[:55]

        classname = sqlescape(furni["classname"].encode(
            "latin-1", "ignore").decode("latin-1"))
        
        if furni_name == "":
            furni_name = classname 
        ...
        f.write(
            f"UPDATE catalog_items ci, (SELECT CAST(id AS CHAR) as id, item_name FROM furniture WHERE item_name = '{classname}') item SET ci.catalog_name = '{furni_name}' WHERE ci.item_id = item.id;\n")
