! function(t) {
    ...
    t.exports = "<transition name=opacity> <div class=notif_alert v-if=\"data.connected && data.notif_alert_open\" movebox> <div class=head v-draggable> <div class=croix v-on:click=Close>×</div> {{data.notif_alert_title}} </div> <div class=box_body style=padding:5px> <div class=col_right> <img v-if=\"data.notif_alert_image != ''\" v-bind:src=\"'"+window.location.origin+"/swfs/c_images/notifications/' + data.notif_alert_image + '.gif'\"/> </div> <div class=col_left> <div class=message v-html=data.notif_alert_message> </div> <button type=button class=box_button v-on:click=JoinRoom>{{data.notif_alert_textbutton}}</button> </div> </div> </div> </transition>"
}, function(t, e) {
    ...
    t.exports = '<div class=bot_choose v-if="data.connected && data.rp_mode" v-show="data.in_room && data.rp_botchoose.length > 0"> <ul> <li v-for="choose in GetChoose" v-on:click=Click(choose.code)> <img v-if="choose.look != \'\'" v-bind:src="\'https://habbo.city/habbo-imaging/avatarimage?figure=\' + choose.look + \'&size=s&headonly=true\'" class=head> <span class=name v-if="choose.username != \'\'">{{ choose.username }} :</span> <span class=message>{{ choose.message }}</span> </li> </ul> </div>'
}, function(t, e, n) {
    t.exports = n(35)
}, function(t, e, n) {
    (function(e, n) {
                        ...
                        switch (l.label) {
                            ...
                            case 1:
                                return n = l.sent(), r = n.data, i = Math.round((new Date).getTime() / 1e3), a = r.cache, s = {
                                    ...
                                    "site.url": r.UrlJavvo,
                                    "url.prefix":  window.location.origin + "/swfs",
                                    "client.reload.url":  window.location.origin,
                                    "client.fatal.error.url":  window.location.origin,
                                    "client.connection.failed.url":  window.location.origin,
                                    "logout.url": "/logout",
                                    "logout.disconnect.url":  window.location.origin,
