...
if(isset($_GET["type"]) && isset($_GET["password"]) && isset($_GET["username"])){
    $rcon = new Rcon("ip", 30001, $_GET["password"]);
    ...
    if($_GET["type"] == "builderTool"){
        ...
        echo "CreateLinkEvent('bawtool/show')";
        exit();
    }
    ...
    if($_GET["type"] == "setColors"){
        $user->saveColorsBySso($_GET["colorPrimary"], $_GET["colorText"], $_GET["colorSecondary"], $_GET["sso"]);
        exit();
    }    
