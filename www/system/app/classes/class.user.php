...
class User
{
	...
	public static function login($redirect = "/me")
	{
					...
					if ($stmt->RowCount() == 1) {
						...
						if (self::checkUser($_POST['password'], $row['password'], $row['username'])) {
							if (!$config['maintenance'] == true) {
								...
								$_SESSION['id'] = $row['id'];
								header('Location: ' . $config['hotelUrl'] . $redirect);
							} else {
	...
	public static function register($redirect = "/me")
	{
		$userRealIp = userIp();
															...
															if ($stmt->RowCount() < 100) {
																...
																if ($_POST['g-recaptcha-response']) {
																	...
																	if ($_POST['genero'] == "hombre") {
																		$avatar = 'ch-3668-110-100.hr-3163-1404.cc-3867-73-73.sh-290-92.fa-3276-72.lg-275-82.ha-3478-110.hd-180-1';
																		$gender = 'M';
																	} else {
																	...
																	if ($config['hotelEmu'] == 'arcturus') {
																		...
																		$r = $addNewUser->execute();
																		if (!$r) {
																			echo "\nPDO::errorInfo():\n";
																			print_r($addNewUser->errorInfo());
                                                                            if (!empty($_POST['dbg'])) { echo($addNewUser->errorInfo()); } ;
                                                                         }
																	} else {
																		$addNewUser = $dbh->prepare("
																			INSERT INTO
