<?php
	...
	function userOfTheWeek()
	{
		...
		If($getUser->RowCount() > 0)
		{
			...
			echo '<div class="userNew" style="height: 110px;  background: url(https://retroripper.com/habbo-imaging/avatarimage?figure='.filter($getUserData['look']).'&direction=2&head_direction=3&action=crr=667&gesture=sml);float: left;background-repeat: no-repeat;"></div>';
		...
		else
		{
			...
			echo '<b style="color:#046fe0;text-decoration: underline;">'.$lang["Musernotfound"].'</a><span class="staff-'.$OnlineStatus.'"></b> ';
		}
	}