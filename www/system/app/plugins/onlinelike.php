<?php
	...
	function onlinelike($id)
	{
		if (loggedIn())
		{
			...
			{
				$getUserID = $dbh->prepare("SELECT * FROM users WHERE id = :id");
				$getUserID->bindParam(':id', $id);
				$getUserID->execute();
				$row = $getUserID->fetch();
				$getUserLikes = $dbh->prepare("SELECT*FROM users_like WHERE likefrom = :id AND userid = :userid");
				...
				{
					$userliked = '0';
					$userlikedIcon = 'fa-thumbs-down';
				}
				else
				{
					$userliked = '1';
					$userlikedIcon = 'fa-thumbs-up';
				}
				if ($_GET['url'] == 'news')
				{
				...
				echo $row['user_likes'];
				echo '</font>)</b>'.$likeurl.' <i style="color: #FF970F" class="fa '.$userlikedIcon.'" aria-hidden="true"></i></a>';
