(function ($) {
  ...
  $("#avatarSelector").on("click", "a[data-clothing]", function () {
    ...
    var color =
      typeof current.color === "undefined" || current.color == ""
        ? "61"
        : current.color;
  ...
  function AvatarGenerate() {
    ...
    this.switchGender = function () {
      this.gender =
        (this.gender !== "M" && this.gender !== "F") || this.gender == "F"
          ? "M"
          : "F";
    };
    ...
    this.buildFigure = function (usingHabbo) {
      ...
      jQuery.each(
        this.getGender() == "M" ? habbo : habboFemale,
        function (index, value) {
          if (value.set == "" || value.color == "") return;
    ...
    this.importFigure = function (figure) {
      ...
      jQuery.each(arrayFigure, function (index, value) {
        ...
        habbo[array[0]].color =
          3 in array ? array[2] + "-" + array[3] : array[2];
      });
    };
    ...
    this.loadToClothes = function (type) {
      ...
      jQuery.each(load.sets, function (index, value) {
        if (
          (value.gender == _this.gender && value.selectable == 1) ||
          (value.gender == "U" && value.selectable == 1)
        ) {
          var currentlyactive = _this.getPart(type);
          var partString = type + "-" + index + "-" + currentlyactive.color;

          var asset = $(
            '<a href="#" class="page-content-choose-style-wrapper-item-content-look" data-clothing="' +
              index +
              '"><span class="page-content-choose-style-wrapper-item-content-look-figure" style="background-image: url(/assets/images/client/figure/' +
              type +
              "-" +
              index +
              '.png);"></span></a>'
    ...
    this.loadToColors = function (type) {
      ...
      jQuery.each(colors, function (index, value) {
        if (value.selectable == 1) {
          var color = $(
            '<a href="#" class="page-content-choose-style-wrapper-item-content-color" style="background-color: #' +
              value.hex +
              '" data-palette="' +
              load.paletteid +
              '" data-color="' +
    ...
    this.updateAvatar = function () {
      var URL =
        "https://www.habbo.com.tr/habbo-imaging/avatarimage?head_direction=4&direction=4&size=l&figure=" +
        this.buildFigure() +
        "&gender=" +
        this.getGender();
