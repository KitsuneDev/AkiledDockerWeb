...
?>
			...
			<?php
			$getRanks = $dbh->prepare("SELECT * FROM permissions_groups WHERE id > 2 and id < 7  ORDER BY id DESC");
			$getRanks->execute();
			while ($Ranks = $getRanks->fetch())
			{	
				echo '
		   						<div style="font-size: 24px;background: rgba(39, 39, 39, 0.92);border-radius: 3px 3px 0px 0px;padding: 5px;padding-left: 30px;border-bottom: 2px solid #404040;">' . $Ranks['name'] . '</div>
				...
				';
				$getMembers = $dbh->prepare("SELECT id,username,motto,look,online,tarea FROM users WHERE rank = :ranid");
...
<td style="padding-left: 15px;background: rgba(34, 34, 34, 0.62);"><p style=" font-size: 17px; ">'.$lang["HkRankTarea"].': ' .$tarea . '</p></td>
		...
		?>
						...
						<div style="font-size: 24px;background: rgba(39, 39, 39, 0.92);border-radius: 3px 3px 0px 0px;padding: 5px;padding-left: 30px;border-bottom: 2px solid #404040;"><?=$lang["staffalfa"]?></div>
						<fieldset class="form__fieldset form__fieldset--box" style="margin: 0;border-radius: 0px 0px 3px 3px;padding: 10px;margin-bottom: 20px;">


				<div style="padding: 20px;font-size: 17px;margin-top: -20px;margin-bottom: -20px;"><?= $lang["alfainfo"]?></div>
				            <div class="col-lg-4">
                <img src="/templates/<?= $config['skin']; ?>/assets/images/staff.png">
            </div>
				</fieldset>
						</div>
                    </div>
